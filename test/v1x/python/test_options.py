...
class TestOptions(unittest.TestCase):
    ...
    def test_nok(self):
        ...
        self.assertEqual(str(cm.exception).split('\n', maxsplit=1)[0],
                         'Usage error: invalid value for yes/no option optimize: nope')

        with self.assertRaises(Exception) as cm:
            ql.set_option('scheduler', 'best')

        self.assertEqual(str(cm.exception).split('\n', maxsplit=1)[0],
                         'Usage error: invalid value for option scheduler: '
                         'possible values are ASAP or ALAP, but best was given')
