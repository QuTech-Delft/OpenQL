...
namespace ql::pass::map::qubits::map {
...
protected:
    ...
    static void checkCircuitSemanticsAreTheSame(const ir::Ref &ir1, const ir::Ref &ir2) {
        ...
        ASSERT_FALSE(ir1->platform.empty());
        ASSERT_FALSE(ir2->platform.empty());
        EXPECT_TRUE(ir1->platform->equals(*ir2->platform));
        ASSERT_EQ(ir1->program->blocks.size(), 1);
        ASSERT_EQ(ir2->program->blocks.size(), 1);
        ...
        EXPECT_TRUE(ddgs_are_equal(ir1->program->blocks[0], ir2->program->blocks[0]));
    }

    static void deswapCircuit(const ir::Ref &ir) {
        ASSERT_EQ(ir->program->blocks.size(), 1);
        ...
        for (const auto &st: ir->program->blocks[0]->statements) {
            ...
            ASSERT_FALSE(custom_instr.empty());
...
toffoli q[1], q[2], q[3]
    ...
    EXPECT_EQ(exceptionMessage.rfind(
        "Internal compiler error: Mapper/router does not handle gates with more than 2 qubits operands.", 0), 0);
}


TEST_F(MapTest, this_test_class_does_not_handle_swaps_that_do_not_end_up_in_the_circuit) {
    auto circuit = R"(
version 1.2
...
cnot q[4], q[3]
    ...
    ASSERT_EQ(output->program->blocks[0]->statements.size(), 1);

    auto instr = output->program->blocks[0]->statements[0].as<ir::CustomInstruction>();
    ASSERT_FALSE(instr.empty());

    EXPECT_EQ(instr->instruction_type->name, "cnot");

    ir::OperandsHelper ops(output->platform, *instr);
    auto qops = ops.get2QGateOperands();

    EXPECT_EQ(qops.first, 4);
    EXPECT_EQ(qops.second, 1);
}


TEST_F(MapTest, simple_s7) {
    auto circuit = R"(
version 1.2
...
class MapLotOfCzsOnS7Test : public MapTest {
protected:
...
TEST_F(MapLotOfCzsOnS7Test, base_route_heuristic) {
    set_option("route_heuristic", "base");
    runAndCheck(circuit.str());
}


TEST_F(MapLotOfCzsOnS7Test, min_extend_route_heuristic) {
    set_option("route_heuristic", "minextend");
    runAndCheck(circuit.str());
}


class MapLotOfCnotsOnS17Test : public MapTest {
protected:
    std::stringstream circuit{};
...
TEST_F(MapLotOfCnotsOnS17Test, base_route_heuristic) {
    set_option("route_heuristic", "base");
    runAndCheck(circuit.str());
}


TEST_F(MapLotOfCnotsOnS17Test, min_extend_route_heuristic) {
    set_option("route_heuristic", "minextend");
    runAndCheck(circuit.str());
}
