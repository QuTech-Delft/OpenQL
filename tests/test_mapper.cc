...
int main(int argc, char ** argv)
{
    ...
    std::string kernel_name = "test1" ;

    ql::quantum_platform starmon("cc_light17", "qs.json");
    ql::set_platform(starmon);
    ql::quantum_program prog(prog_name, starmon, n, 0);
    ql::quantum_kernel k(kernel_name, starmon, n, 0);
	...
	// std::cout << prog.kernels.at(0).c.at(0);
	// IOUT(prog.kernels.at(0).qasm());
	
	IOUT("Fidelity: " + std::to_string(ql::metrics::quick_fidelity_circuit(prog.kernels.at(0).c)));
	//It still stays that the gates are being executed at INT_MAX


	// IOUT("Gate duration" + std::to_string(prog.kernels.at(0).c.at(0)->duration));
	// IOUT("Gate cycle" + std::to_string(prog.kernels.at(0).c.at(0)->cycle));
	// IOUT(k.qasm());
	// IOUT(prog.kernels.at(0).qasm());
	// IOUT("Hey there!!!");
