...
#else
            tSignalInfo si = findSignalInfoForQubit(instructionSignalType, qubit);
            const json &ccSetupSlot = jsonCcSetup["slots"][si.slotIdx];
...
//FIXME    json jsonCcSetup;
    ...
    void latencyCompensation()
    {
        ...
        const json &ccSetupSlots = jsonCcSetup["slots"];    // FIXME: old syntax
        for(size_t slotIdx=0; slotIdx<slotsUsed; slotIdx++) {            // iterate over slot vector
            // collect info from JSON
            const json &ccSetupSlot = ccSetupSlots[slotIdx];
            int slot = ccSetupSlot["slot"];
            const json &instrument = ccSetupSlot["instrument"];
