...
//                ssclassical << ", " + std::to_string(classical_ins->imm_value);
    ...
    void emit(std::stringstream &ss, const char *s)
    {
        ss << s << std::endl;
    }

    void emit(std::stringstream &ss, const char *instr, std::ostringstream &ops, const char *comment)
    {
        ss << "        " << setw(8) << instr << ops << setw(0) << comment << std::endl;
    }
    ...
    std::string get_prologue(ql::quantum_kernel &k)
    {
        ...
        switch(k.type) {
            ...
            case kernel_type_t::FOR_START:
                ...
                emit(ss, "move", OSS(k.iterations << ",R63"), "# R63 is for loop counter");        // FIXME: fixed reg, no nested loops
                break;
    ...
    std::string get_epilogue(ql::quantum_kernel &k)
    {
        ...
        switch(k.type) {
            ...
            case kernel_type_t::FOR_END:
            {
                ...
                emit(ss, "# FOR_END");
                emit(ss, "loop", OSS("R63,@" << tokens[0]), "# R63 is for loop counter");        // FIXME: fixed reg, no nested loops
                break;
            }
