...
private:
    ...
    /**
     * Total number of cycles saved per block.
     */
     ...
     * reset state.
     */
    void sync_all(const ir::Ref &ir, const ir::BlockRef &b);
    void sync(const ir::Ref &ir, const ir::BlockRef &b, unsigned long q);
     ...
     */
    static utils::Int statement2cs(const ir::InstructionType &instructionType);
...
public:
     ...
     */
    utils::UInt optimize_blocks(const ir::Ref &ir);

private:

    /**
     * Optimizes the one block of the kernel kernel, returning how many cycles were saved.
     */
    utils::UInt optimize_one_block(ir::BlockRef &block, const ir::Ref &ir);

    static utils::Int qubit_nr(utils::One <ir::Expression> &operand);

    static void add_instruction(const ir::Ref &ir, const ir::BlockRef &block, const utils::Str &gate_name, unsigned long qubit);

    static void add_clifford_instruction(const ir::Ref &ir, const ir::BlockRef &b, long id, unsigned long qubit);

    static void  rx90(const ir::Ref &ir, const ir::BlockRef &b, unsigned long qubit);
    static void  ry90(const ir::Ref &ir, const ir::BlockRef &b, unsigned long qubit);
    static void mrx90(const ir::Ref &ir, const ir::BlockRef &b, unsigned long qubit);
    static void mry90(const ir::Ref &ir, const ir::BlockRef &b, unsigned long qubit);
    static void rx180(const ir::Ref &ir, const ir::BlockRef &b, unsigned long qubit);
    static void ry180(const ir::Ref &ir, const ir::BlockRef &b, unsigned long qubit);
