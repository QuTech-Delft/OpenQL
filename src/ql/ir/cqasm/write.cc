...
protected:
    ...
    utils::Bool version_equal(const utils::Vec<utils::UInt> &rhs) {
        return version_compare(options.version, rhs) == std::strong_ordering::equal;
    }
    utils::Bool version_less_than(const utils::Vec<utils::UInt> &rhs) {
        return version_compare(options.version, rhs) == std::strong_ordering::less;
    }
    utils::Bool version_more_than(const utils::Vec<utils::UInt> &rhs) {
        return version_compare(options.version, rhs) == std::strong_ordering::greater;
    }
    utils::Bool version_less_than_or_equal(const utils::Vec<utils::UInt> &rhs) {
        return !version_more_than(rhs);
    }
    utils::Bool version_more_than_or_equal(const utils::Vec<utils::UInt> &rhs) {
        return !version_less_than(rhs);
    }

    /**
     * Returns whether the target cQASM version is at least the given version.
     */
    utils::Bool version_at_least(const utils::Vec<utils::UInt> &rhs) {
        return version_more_than_or_equal(rhs);
    }
...
public:
    ...
    void visit_root(Root &node) override {
        ...
        // Generate header.
        // QI2 integration test: temporarily always writing out v3 comments
        //if (version_less_than_or_equal({ 1, 2 })) {
        //    os << sl() << "# ";
        //} else if (version_equal({ 3, 0 })) {
            os  << sl() << "// ";
        //}
        ...
        // QI2 integration test: temporarily always writing out v3 version statement
    ...
    void visit_platform(Platform &node) override {
        ...
        // Print the size of the main qubit register for cQASM 1.0 or
        // when registers are to be made explicit for cQASM < 3.0.
        // QI2 integration test: temporarily always writing out v3 qubit statement
        //if (version_less_than_or_equal({1, 0}) || (version_less_than({3, 0}) && options.registers_as_variables)) {
        //    QL_ASSERT(ir->platform->qubits->shape.size() == 1);
        //    os << sl() << "qubits " << ir->platform->qubits->shape[0];
        //    os << el(1);
        //} else if (version_equal({3, 0})) {
            os << sl() << "qubit[" << ir->platform->qubits->shape[0] << "] q";
        //}
    ...
    void visit_program(Program &node) override {
        ...
        for (utils::UInt idx = 0; idx < node.blocks.size(); idx++) {
            ...
            os << el();
            // QI2 integration test: temporarily removed writing out of the block header
            //os << sl(-1) << "." << name;
