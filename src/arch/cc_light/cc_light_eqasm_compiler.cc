...
ir::Instruction classical_cc::qasm() const {
    Str iopers;
    Int sz = creg_operands.size();
    for (Int i = 0; i < sz; ++i) {
        if (i == sz - 1) {
...
ir::GateType classical_cc::type() const {
    return ir::GateType::CLASSICAL;
}

ir::Complex2by2Matrix classical_cc::mat() const {
    return m;
}
...
Str ir2qisa(
    ...
    for (ir::Bundle &abundle : bundles1) {
        ...
        for (; secIt1 != abundle.parallel_sections.end(); ++secIt1) {
            for (auto secIt2 = std::next(secIt1); secIt2 != abundle.parallel_sections.end(); ++secIt2) {
                ...
                if (insIt1 != secIt1->end() && insIt2 != secIt2->end()) {
                    ...
                    if (itype1 == ir::GateType::CLASSICAL || itype2 == ir::GateType::CLASSICAL) {
    ...
    ir::Bundles bundles2;
    for (ir::Bundle &abundle1 : bundles1) {
        ir::Bundle abundle2;
    ...
    for (ir::Bundle &abundle : bundles2) {
        // sorts instructions alphabetically
        abundle.parallel_sections.sort(
            [](const ir::Section &sec1, const ir::Section &sec2) -> Bool {
                auto i1 = sec1.begin();
                auto iname1 = (*(i1))->name;
                auto i2 = sec2.begin();
                auto iname2 = (*(i2))->name;
    ...
    for (ir::Bundle &abundle : bundles2) {
        ...
        for (auto secIt = abundle.parallel_sections.begin(); secIt != abundle.parallel_sections.end(); ++secIt) {
            ...
            if (itype == ir::GateType::CLASSICAL) {
                classical_bundle = true;
                ssinst << classical_instruction2qisa(*firstInsIt->as<classical_cc>());
            } else {
...
void cc_light_eqasm_compiler::compile(ir::Program &program, const quantum_platform &platform) {
    //std::cout << " ============= DEBUG PRINT FOR DEBUG(1): In cc_light BACKEND COMPILER " << std::endl;
    QL_DOUT("Compiling " << program.kernels.size() << " kernels to generate CCLight eQASM ... ");
    ...
    ccl_prep_code_generation(program, platform, "ccl_prep_code_generation");

    // decompose_pre_schedule pass
    // is very much concerned with generation of classical code
    ccl_decompose_pre_schedule(program, platform, "ccl_decompose_pre_schedule");

    // this call could also have been at end of back-end-independent passes
    write_quantumsim_script(program, platform, "write_quantumsim_script_unmapped");

    clifford_optimize(program, platform, "clifford_premapper");
    ...
    latency_compensation(program, platform, "ccl_latency_compensation");

    insert_buffer_delays(program, platform, "ccl_insert_buffer_delays");

    // decompose meta-instructions after scheduling
    ccl_decompose_post_schedule(program, platform, "ccl_decompose_post_schedule");

    // just before code generation, emit quantumsim script to best match target architecture
    write_quantumsim_script(program, platform, "write_quantumsim_script_mapped");

    // and now for real
    if (options::get("generate_code") == "yes") {
        qisa_code_generation(program, platform, "qisa_code_generation");
    }
...
void cc_light_eqasm_compiler::ccl_decompose_pre_schedule_kernel(
    ...
    ir::Circuit decomp_ckt; // collect result circuit in here and before return swap with kernel.c
    ...
    for (auto ins : kernel.c) {
        ...
        if (itype == ir::GateType::CLASSICAL) {
            ...
            if (
                ...
                decomp_ckt.add(make_node<classical_cc>(iname, icopers));
                QL_DOUT("    classical instruction decomposed: " << decomp_ckt.back()->qasm());
            } else if (
                (iname == "eq") || (iname == "ne") || (iname == "lt") ||
                (iname == "gt") || (iname == "le") || (iname == "ge")
            ) {
                decomp_ckt.add_raw(new classical_cc("cmp", {icopers[1], icopers[2]}));
                QL_DOUT("    classical instruction decomposed: " << decomp_ckt.back()->qasm());
                decomp_ckt.add_raw(new classical_cc("nop", {}));
                QL_DOUT("                                      " << decomp_ckt.back()->qasm());
                decomp_ckt.add_raw(new classical_cc("fbr_"+iname, {icopers[0]}));
                QL_DOUT("                                      " << decomp_ckt.back()->qasm());
            } else if (iname == "mov") {
                // r28 is used as temp, TODO use creg properly to create temporary
                decomp_ckt.add_raw(new classical_cc("ldi", {28}, 0));
                QL_DOUT("    classical instruction decomposed: " << decomp_ckt.back()->qasm());
                decomp_ckt.add_raw(new classical_cc("add", {icopers[0], icopers[1], 28}));
                QL_DOUT("                                      " << decomp_ckt.back()->qasm());
            } else if (iname == "ldi") {
            ...
            if (iname == "wait") {
                ...
                if (is_measure) {
                    ...
                    auto qop = iqopers[0];
                    decomp_ckt.add(ins);
                    if (itype == ir::GateType::CUSTOM) {
                        auto &coperands = ins->creg_operands;
                        if (!coperands.empty()) {
                            auto cop = coperands[0];
                            decomp_ckt.add_raw(new classical_cc("fmr", {cop, qop}));
                        } else {
...
void cc_light_eqasm_compiler::qisa_code_generation(
    ...
    Str unique_name = program.unique_name;
...
void cc_light_eqasm_compiler::write_quantumsim_program(
    ...
    Str qfname( options::get("output_dir") + "/" + "quantumsim_" + program.unique_name + "_" + suffix + ".py");
    ...
    fout << "    c = Circuit(title=\"" << program.unique_name << "\")\n";
    ...
    QL_ASSERT(program.kernels.size() <= 1);
    Vec<UInt> check_usecount;
    check_usecount.resize(count, 0);

    for (auto &gp : program.kernels.front()->c) {
        switch (gp->type()) {
