...
void codegen_cc::program_start(const std::string &progName)
{
    ...
    cccode << "# Program: '" << progName << "'" << std::endl;
...
{
    ql::utils::zero(lastStartCycle);       // FIXME: actually, bundle.startCycle starts counting at 1
}
...
void codegen_cc::bundle_finish(size_t startCycle, size_t durationInCycles, bool isLastBundle)
{
    ...
    for(size_t instrIdx=0; instrIdx<jsonInstruments->size(); instrIdx++) {
        ...
        JSON_ASSERT(jsonInstrument, "controller", instrumentName);
        ...
        for(size_t group=0; group<nrGroups; group++) {                      // iterate over groups used within slot
            if(groupInfo[instrIdx][group].signalValue != "") {              // signal defined, i.e.: we need to output something
                ...
                JSON_ASSERT(*jsonControlModes, controlModeRef, "control_modes")
                const json &jsonControlMode = (*jsonControlModes)[controlModeRef];      // the control mode definition for our instrument
                size_t nrControlBitsGroups = jsonControlMode["control_bits"].size();    // how many groups of control bits does the control mode specify
...
//                        << ", group=" << group
            ...
            padToCycle(lastStartCycle[instrIdx], startCycle, slot, instrumentName);
            ...
            lastStartCycle[instrIdx] = startCycle + slotDurationInCycles;
        ...
        if(isLastBundle) {
            padToCycle(lastStartCycle[instrIdx], startCycle+durationInCycles, slot, instrumentName);
...
void codegen_cc::padToCycle(size_t lastStartCycle, size_t startCycle, int slot, const std::string &instrumentName)
{
    // compute prePadding: time to bridge to align timing
    ssize_t prePadding = startCycle - lastStartCycle;
    if(prePadding < 0) {
    ...
    if(prePadding > 0) {     // we need to align
        emit(SS2S("[" << slot << "]").c_str(),      // CCIO selector
            ...
            SS2S("# cycle " << lastStartCycle << "-" << startCycle << ": padding on '" << instrumentName+"'").c_str());
    }
}
