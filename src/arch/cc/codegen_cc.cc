...
void codegen_cc::init(const quantum_platform &platform)
{
    ...
    if(!map_input_file.empty()) {
        ...
        codewordTable = map["codeword_table"];      // FIXME: use utils::json_get
        mapPreloaded = true;
    }
    ...
    for(size_t instrIdx=0; instrIdx<settings.getInstrumentsSize(); instrIdx++) {
...
#endif
#if OPT_PRAGMA
		const utils::Json *pragma = nullptr;
		int pragmaSmBit = 0;
#endif
        ...
        for(size_t group=0; group<nrGroups; group++) {
			...
			if(bi->readoutQubit >= 0) { // readout requested
				...
				// get our qubit
				const utils::Json qubits = utils::json_get<const utils::Json>(*ic.ii.instrument, "qubits", ic.ii.instrumentName);   // NB: utils::json_get<const utils::Json&> unavailable
		...
		if(pragma) {	// NB: note that this will only work because we set the pragma for all instruments, and thus already encounter this for the first instrument
			...
			int pragmaBreakVal = utils::json_get<int>(*pragma, "break", "pragma of unknown instruction");		// FIXME we don't know which instruction we're dealing with, so better move
...
std::string toQasm(const std::string &iname, const utils::Vec<utils::UInt> &qops, const utils::Vec<utils::UInt> &cops)
{
	std::stringstream s;
	s << iname << " ";
...
codegen_cc::tCalcSignalValue codegen_cc::calcSignalValue(const settings_cc::tSignalDef &sd, size_t s, const utils::Vec<utils::UInt> &qops, const std::string &iname)
{   tCalcSignalValue ret;
    ...
    ret.operandIdx = utils::json_get<unsigned int>(sd.signal[s], "operand_idx", signalSPath);
...
uint32_t codegen_cc::assignCodeword(const std::string &instrumentName, int instrIdx, int group)
{
    ...
    if(QL_JSON_EXISTS(codewordTable, instrumentName) &&                    // instrument exists
        ...
        utils::Json &myCodewordArray = codewordTable[instrumentName][group];
