 ...
 */
static size_t get_classical_operations_count(
    const circuit &c,
...
static size_t get_non_single_qubit_quantum_gates_count(
    const circuit &c,
    const quantum_platform &platform
) {
...
static size_t get_quantum_gates_count(
    const circuit &c,
    const quantum_platform &platform
) {
    size_t quantum_gates = 0;
...
static size_t get_circuit_latency(
    const circuit &c,
    const quantum_platform &platform
) {
    size_t cycle_time = platform.cycle_time;
...
static void report_write_qasm(
    ...
    utils::write_file(fname.str(), out_qasm.str());
 ...
 */
static std::stringstream report_compose_write_name(
 ...
 */
static std::stringstream report_compose_report_name(
    const std::string &unique_name,
...
void report_kernel_statistics(
    ...
    ofs << comment_prefix << "----- qubit cycles use:" << utils::to_string(usedcyclecount) << "\n";
...
void get_kernel_statistics(
    ...
    *ofs += comment_prefix; *ofs += "----- qubit cycles use:"; *ofs += utils::to_string(usedcyclecount); *ofs += "\n";
...
void report_totals_statistics(
    std::ofstream &ofs,
    const std::vector<quantum_kernel> &kernels,
    const quantum_platform &platform,
    const std::string &comment_prefix
)
{
 ...
 */
static int report_bump_unique_file_version(const quantum_program *programp) {
